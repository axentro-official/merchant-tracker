...
function validateMerchantAndMachineForCollection(merchant, machine) {
  if (!merchant) {
    showOperationBlockedToast('يرجى اختيار تاجر صحيح من القائمة');
    return false;
  }
  if (!isMerchantActive(merchant)) {
    showOperationBlockedToast('لا يمكن تنفيذ تحصيل على تاجر غير نشط', `حالة التاجر: ${merchant['الحالة'] || 'غير محددة'}`);
    return false;
  }
  ...
  if (!machine || !isMachineActive(machine)) {
    showOperationBlockedToast('لا يمكن تنفيذ التحصيل إلا على مكنة نشطة', machine ? `حالة المكنة: ${machine['الحالة'] || 'غير محددة'}` : 'لم يتم تحديد مكنة نشطة');
    return false;
  }
  return true;
}
...
function parseMoneySafe(value) {
  ...
  const normalized = normalizeDigits(withoutHtml).replace(/[,،]/g, '');
  const match = normalized.match(/-?\d+(?:\.\d+)?/);
  const parsed = match ? parseFloat(match[0]) : 0;
  return Number.isFinite(parsed) ? parsed : 0;
}

function cleanCollectionNotes(notesValue) {
  return String(notesValue || '').replace(/^\[COLLECTION_TYPE:[^\]]+\]\s*/i, '').trim();
}

function getCollectionType(collection) {
  return collection?.['نوع التحصيل'] || extractCollectionTypeFromNotes(collection?.['ملاحظات']) || 'تحصيل جزئي';
}

function showDebtWarningModal(amount, currentDebt) {
  const existing = document.getElementById('collectionDebtWarningOverlay');
  if (existing) existing.remove();
  const overlay = document.createElement('div');
  overlay.id = 'collectionDebtWarningOverlay';
  overlay.setAttribute('role', 'dialog');
  overlay.setAttribute('aria-modal', 'true');
  overlay.innerHTML = `
    <div class="collection-debt-warning-card">
      <div class="collection-debt-warning-icon"><i class="fas fa-triangle-exclamation"></i></div>
      <h3>مبلغ التحصيل غير صحيح</h3>
      <p>مبلغ التحصيل أكبر من المديونية الحالية للتاجر، لذلك لم يتم حفظ العملية.</p>
      <div class="collection-debt-warning-grid">
        <div><span>مبلغ التحصيل المدخل</span><strong>${formatMoneyText(amount)}</strong></div>
        <div><span>المديونية الفعلية الحالية</span><strong>${formatMoneyText(currentDebt)}</strong></div>
      </div>
      <button type="button" class="collection-debt-warning-btn">تعديل المبلغ</button>
    </div>
  `;
  document.body.appendChild(overlay);
  const close = () => overlay.remove();
  overlay.querySelector('.collection-debt-warning-btn')?.addEventListener('click', close);
...
async function setCollectionMerchant(merchant, selectedMachine = '', excludeId = null) {
  if (merchant && !isMerchantActive(merchant)) {
    showOperationBlockedToast('لا يمكن اختيار تاجر غير نشط لإجراء تحصيل', `حالة التاجر: ${merchant['الحالة'] || 'غير محددة'}`);
    merchant = null;
  }
...
function renderCollectionsTable() {
  ...
  if (!currentCollections.length) {
    tbody.innerHTML = '<tr><td colspan="11" class="empty-state">لا توجد تحصيلات</td></tr>';
    return;
  }
  tbody.innerHTML = currentCollections.map((collection, idx) => {
    const collectionType = getCollectionType(collection);
    const remaining = parseMoneySafe(collection['المتبقي بعد التحصيل']);
    const cleanNotes = cleanCollectionNotes(collection['ملاحظات']) || '-';
    return `
      <tr>
        <td>${idx + 1}</td>
        ...
        <td><span class="badge badge-info">${escapeHtml(collectionType)}</span></td>
        <td>${formatMoney(parseMoneySafe(collection['قيمة التحصيل']))}</td>
...
export async function saveCollection() {
  ...
  try {
    ...
    const machine = machineByCode(document.getElementById('collMachineId').value);
    if (!validateMerchantAndMachineForCollection(merchant, machine)) return;
    ...
    if (amount > currentDebt) {
      showDebtWarningModal(amount, currentDebt);
      return;
    }
    const { date, time } = getNowDateTime();
    const collectionType = normalizeCollectionType(document.getElementById('collType').value || 'تحصيل جزئي');
    const payload = {
      ...
      'نوع التحصيل': collectionType,
      'ملاحظات': buildCollectionTypeTaggedNotes(collectionType, document.getElementById('collNotes').value.trim()),
      'التاريخ': date,
      'الوقت': time,
