...
export function initMachinesPage() { supabase = window.supabaseClient; }
...
function fillMerchantDatalist() {
  ...
  if (!dl) return;
  dl.innerHTML = merchantsList.map(m => `<option value="${escapeHtml(m['رقم التاجر'])}">${escapeHtml(m['رقم التاجر'])} - ${escapeHtml(m['اسم التاجر'])}</option>`).join('');
}
...
function attachMerchantSearch() {
  ...
  search.onchange = search.oninput = () => {
    const merchant = merchantsList.find(m => m['رقم التاجر'] === search.value || `${m['رقم التاجر']} - ${m['اسم التاجر']}` === search.value);
...
export async function loadMachines() {
  ...
  if (!supabase) return;
  try {
    const { data, error } = await supabase.from('machines').select('*').order('updated_at', { ascending: false });
    if (error) throw error;
    currentMachines = data || [];
    await ensureMerchants();
    renderMachinesTable();
  } catch (err) { console.error(err); showToast('خطأ في تحميل المكن', 'error'); }
}

function renderMachinesTable() {
  const tbody = document.getElementById('machinesTableBody');
  if (!tbody) return;
  if (!currentMachines.length) { tbody.innerHTML = '<tr><td colspan="8" class="empty-state">لا توجد مكن مسجلة</td></tr>'; return; }
  tbody.innerHTML = currentMachines.map((m, idx) => `
    <tr>
      ...
      <td><div class="action-btns"><button class="btn btn-primary btn-sm" onclick="window.editMachine('${m.id}')"><i class="fas fa-edit"></i></button><button class="btn btn-danger btn-sm" onclick="window.deleteMachine('${m.id}')"><i class="fas fa-trash"></i></button></div></td>
    </tr>`).join('');
}
...
export async function openMachineModal(machine = null) {
  ...
  document.getElementById('machineModalTitle').innerHTML = machine ? '<i class="fas fa-edit"></i> تعديل مكنة' : '<i class="fas fa-plus-circle"></i> إضافة مكنة جديدة';
  ...
  document.getElementById('machMerchantSearch').value = merchant ? merchant['رقم التاجر'] : '';
  document.getElementById('machMerchantId').value = merchant?.id || '';
  document.getElementById('machineModal').classList.add('show');
}

export function closeMachineModal() { document.getElementById('machineModal')?.classList.remove('show'); }
...
export async function saveMachine() {
  ...
  try {
    const query = id ? supabase.from('machines').update(payload).eq('id', id) : supabase.from('machines').insert([payload]);
  ...
  } catch (err) { console.error(err); showToast(err.message || 'فشل حفظ المكنة', 'error'); }
}

export async function editMachine(id) {
  const machine = currentMachines.find(m => m.id === id);
  if (machine) return openMachineModal(machine);
}
