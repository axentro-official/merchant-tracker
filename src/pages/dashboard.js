...
// دالة مساعدة لتحديث قيمة عنصر HTML مع تأثير تزايدي
...
// عرض آخر العمليات (تحويلات + تحصيلات)
    ...
    try {
        ...
        tbody.innerHTML = recent.map((act, idx) => {
            ...
            return `
                <tr>
                    <td>${idx+1}  
                    <td><code class="ref-code">${act.ref || '-'}</code>  
                    <td><span class="badge ${badgeClass}">${act.type}</span>  
                    <td>${act.merchantName || '-'}  
                    <td>${act.activityName || '-'}  
                    <td>${amount} ج.م  
                    <td>${act.date || '-'}  
...
// عرض أعلى المكن تحقيقاً للتارجت
        ...
        tbody.innerHTML = topList.map((m, idx) => {
            ...
            return `
                <tr>
                    ...
                    <td>${m['اسم التاجر'] || '-'}  
                    <td>${m['اسم النشاط'] || '-'}  
                    <td><code class="ref-code">${m['رقم المكنة'] || '-'}</code>  
                    <td><code>${m['الرقم التسلسلي'] || '-'}</code>  
                    <td>${m.target.toLocaleString()} ج.م  
                    <td class="${percentClass}">${m.achieved.toLocaleString()} ج.م  
                    <td>
